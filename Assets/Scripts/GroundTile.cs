...
public class GroundTile : MonoBehaviour
{
	...
	int tracks;
	public double chanceOfObstacle = 0.5;
	...
	void SpawnObstacle(int obstacleTrack)
	{
		...
		Vector3 spawnPosition = new Vector3(-5F + trackSize / 2F + obstacleTrack * trackSize, 0.4F, 5F);

		// Spawn the obstacle
		// Make this the parent of the obstacle, so it gets destroyed with it
		GameObject obstacle = Instantiate(obstaclePrefab, transform, false);
